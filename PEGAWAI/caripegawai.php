...
<body>
					...
					<div class="col s12 m12 l12 card-panel z-depth"> <br>
						<table class="highlight">
							...
							<?php 
			                    ...
			                    $sql = "SELECT * FROM pegawai WHERE ID_Pegawai LIKE '%$cari%' OR NIK LIKE '%$cari%' OR Nama LIKE '%$cari%' OR Tanggal_Lahir LIKE '%$cari%' OR ID_Jenis_Kelamin LIKE '%$cari%' OR Kode_Bagian LIKE '%$cari%'";
			                    $query = mysqli_query($conn,$sql);
			                      ...
			                      if($query!=NULL){
			                        ...
			                        echo "<td><h6>".$data['Kode_Bagian']."</h6></td>";   
				                     echo "<td> <a href='editpegawai.php?id=$data[ID_Pegawai]' style='text-decoration: none;'><i class='material-icons' title='Edit $test'>mode_edit</i></a> | <a data-id='1' class='hapus' href='delete.php?id=$data[ID_Pegawai]' style='text-decoration: none;'><i class='material-icons' title='Hapus $test'>delete</i></a> | <a href='editpassword.php?id=$data[ID_Pegawai]' style='text-decoration: none;'><i class='material-icons' title='Ganti Kata Sandi $test'>lock</i></a> </td></tr>";
				                 }
			                      }else{
			                      	echo "<table>";
						...
						<table>
							<tr>
				            	<td colspan='9'>
				            		<a href='pegawai.php' class="right waves-effect waves-light btn blue darken-2">KEMBALI</a>
				            	</td>
				            </tr>
						</table>
					</div>
	...
	<script>
        ...
        var jawab = confirm("Anda Yakin Ingin Menghapus Pegawai ?");
        if (jawab === true) {
        // konfirmasi
            var hapus = false;
            if (!hapus) {
                hapus = true;
                $.post('deletepegawai.php', {id: $(this).attr('data-id')},
                function (data) {
                    alert(data);
                });
                hapus = false;
