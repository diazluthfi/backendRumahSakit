...
<body>
					...
					<div class="col s12 m12 l12">
						<form name="cari" method="post" action="" class="row">
	                    	<div class="e-input-field col s12 m12 l12">
	                    		<input type="text" name="cari" placeholder="Cari Berdasarkan ID Dokter / Nama / NIK / Alamat / Spesialis  " class="validate" required title="Cari User">
					...
					<div class="col s12 m12 l12 card-panel z-depth"> <br>
						<table class="highlight">
							...
							<?php 
			                    ...
			                    $sql = "SELECT * FROM dokter WHERE ID_Dokter LIKE '%$cari%' OR NIK LIKE '%$cari%' OR Nama LIKE '%$cari%' OR Tanggal_Lahir LIKE '%$cari%' OR ID_Jenis_Kelamin LIKE '%$cari%' OR Kode_Spesialis LIKE '%$cari%'";
			                    $query = mysqli_query($conn,$sql);
			                      ...
			                      if($query!=NULL){
			                        ...
			                        echo "<td><h6>".$data['Kode_Spesialis']."</h6></td>";   
				                     echo "<td> <a href='editdokter.php?id=$data[ID_Dokter]' style='text-decoration: none;'><i class='material-icons' title='Edit $test'>mode_edit</i></a> | <a data-id='1' class='hapus' href='deletedokter.php?id=$data[ID_Dokter]' style='text-decoration: none;'><i class='material-icons' title='Hapus $test'>delete</i></a> | <a href='editpassworddokter.php?id=$data[ID_Dokter]' style='text-decoration: none;'><i class='material-icons' title='Ganti Kata Sandi $test'>lock</i></a> </td></tr>";
				                 }
			                      }else{
			                      	echo "<table>";
						...
						<table>
							<tr>
				            	<td colspan='9'>
				            		<a href='dokter.php' class="right waves-effect waves-light btn blue darken-2">KEMBALI</a>
				            	</td>
				            </tr>
						</table>
					</div>
	...
	<script>
        ...
        var jawab = confirm("Anda Yakin Ingin Menghapus dokter ?");
        if (jawab === true) {
        // konfirmasi
            var hapus = false;
            if (!hapus) {
                hapus = true;
                $.post('deletedokter.php', {id: $(this).attr('data-id')},
                function (data) {
                    alert(data);
                });
                hapus = false;
